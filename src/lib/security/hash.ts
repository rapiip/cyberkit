...
/**
 * MD5 is not available through Web Crypto, so CyberKit ships this
 * implementation. It is the single source of truth for MD5 in the codebase; the
 * byte-oriented form is the primitive and the string form encodes as UTF-8
 * before hashing.
 *
 * MD5 is offered for legacy interoperability and file identification only. It
 * is not collision resistant and must not be used for integrity or signatures.
 */
export function md5Bytes(bytes: Uint8Array): string {
  ...
  // MD5 appends the message length as a little-endian 64-bit integer. The low and
  // high words must be derived separately: JavaScript shift counts are taken
  // mod 32, so `bitLength >>> 32` is `bitLength >>> 0` and would repeat the low
  // bytes instead of emitting the high word.
  const lowWord = bitLength >>> 0;
  const highWord = Math.floor(bitLength / 0x1_0000_0000) >>> 0;
  for (let index = 0; index < 4; index += 1) input.push((lowWord >>> (index * 8)) & 0xff);
  for (let index = 0; index < 4; index += 1) input.push((highWord >>> (index * 8)) & 0xff);
...
/** UTF-8 encodes the input, then hashes it with {@link md5Bytes}. */
export function md5(input: string): string {
  return md5Bytes(textEncoder.encode(input));
}
