...
export const sha1GeneratorTool: ToolDefinition = {
  ...
  execute: async (inputs) => {
    const result = await hashViaWebCrypto('SHA-1', inputs.input as string);
...
export const sha256GeneratorTool: ToolDefinition = {
  ...
  execute: async (inputs) => {
    const result = await hashViaWebCrypto('SHA-256', inputs.input as string);
...
export const sha512GeneratorTool: ToolDefinition = {
  ...
  execute: async (inputs) => {
    const result = await hashViaWebCrypto('SHA-512', inputs.input as string);
...
export const passwordStrengthTool: ToolDefinition = {
  ...
  execute: async (inputs) => {
    const pw = inputs.password as string;
    let score = 0;
...
export const hashIdentifierTool: ToolDefinition = {
  ...
  execute: async (inputs) => {
    const hash = (inputs.hash as string).trim();
