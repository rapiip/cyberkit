...
const challenges = [
  ...
  { 
    ...
    hint: "Use UNION SELECT to join and extract credentials from the 'users' table.", 
  ...
  const simulateLogin = () => {
    ...
    if (activeChallenge === 0) {
      ...
      if (hasOr) {
        ...
        setResult({
          success: false,
          message: '❌ Login Failed. Try using an OR payload like: \' OR \'1\'=\'1',
          query,
        });
      }
      ...
      if (hasComment) {
        ...
        setResult({
          success: false,
          message: '❌ Login Failed. Log in as admin using the comment payload to ignore the password condition.',
          query,
        });
      }
      ...
      if (hasUnion) {
        ...
        setResult({
          success: true,
          message: '🎉 Level 3 Solved! UNION SELECT Attack Successful! You successfully appended rows from the sensitive credentials table.',
          query,
          extractedData,
        });
      } else {
        setResult({
          success: false,
          message: '❌ Query Error or No Data. Use a UNION SELECT query to extract records from the users database table.',
          query,
        });
      }
    }
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-zinc-400 mt-1">
          Practice standard SQL Injection payloads on a simulated database login portal.
        </p>
      </motion.div>
